...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (!target) return errorReply('<:warning:1531049700520624278> Please mention a member. Usage: `-untimeout @user [reason]`');
            ...
            if (logChannel) {
                const logContainer = new ContainerBuilder().addTextDisplayComponents(
                    new TextDisplayBuilder().setContent(
                        `## <:ShieldCheck:1530775133713731826> Untimeout Command Used! | Case #${caseNumber}\n` +
                        `-# **<:sig:1530774414436729012> Used By:** ${message.author}\n` +
                        `**<:user:1530778349184618627> User Untimed Out:** ${target.user.tag} (${target.id})\n` +
